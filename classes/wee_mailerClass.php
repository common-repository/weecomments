...
class wee_mailerClass
{
	...
	function wee_sendScheduledMails()
	{
		...
		if ($orders) {
			...
			foreach ($orders as $order) {
                ...
                //Si no hay email o no tiene formato correcto lo salta
                if (strlen($order->email) < 3 || !filter_var($order->email, FILTER_VALIDATE_EMAIL)) {
                    continue;
                }
				...
				if ((int)$send_mail->send_mail) {					
					...
					if (isset($order->iso_code) && strlen($order->iso_code)>=2) {
						$order->iso_code = substr($order->iso_code, 0, 2);
					} else {
	...
	function wee_prepareMailTemplate($order, $id_products_string)
	{
        ...
        $logoHidden = $this->weecomments_options['WEE_MAIL_LOGOHIDDEN'];
        
        if (isset($logoHidden) && $logoHidden) {
            $htmlLogo = 
                '<tr>
                    <td valign="top" style="padding-right:9px;padding-left:9px;padding-top:0;padding-bottom:0;text-align:center">
                        <a href="'.$link.'" target="_blank">
                            <img align="center" src="'.$this->weecomments_options['WEE_LOGO_URL'].'" style="margin-top:40px; max-width:300px;padding-bottom:0;display:inline!important;vertical-align:bottom;border:0;min-height:auto;outline:none;text-decoration:none" />
                        </a>
                    </td>
                </tr>';
        } else {
        ...
        $weemailClass->wee_registrarCampo("LOGO", $htmlLogo);
	...
	{
		//WPML meta value wpml_language
		
		$sql = "SELECT O.ID as id_order, O.post_date_gmt as date, M.meta_value as email, M2.meta_value as firstname, M3.meta_value as lastname, M4.meta_value as iso_code
		FROM ".$prefix."posts O 
    ...
    function wee_getNumDailyOrders($date, $prefix)
    {
		$sql = "SELECT COUNT(*) as num_daily_orders 
		FROM ".$prefix."posts 
		WHERE post_type = 'shop_order' AND post_status = 'wc-completed' AND date(post_date_gmt) = '".$date."'";
		global $wpdb;
		return $wpdb->get_var($sql);
    }
	
	function wee_getDailyOrdersSales($date, $prefix)
    {
		$sql = "SELECT COALESCE(sum(M.meta_value), 0) as orders_sales
		FROM ".$prefix."posts P LEFT JOIN ".$prefix."postmeta M ON M.post_id = P.ID AND M.meta_key = '_order_total'
		WHERE P.post_type = 'shop_order' AND P.post_status = 'wc-completed' AND DATE(P.post_date_gmt) = '".$date."'";
	...
	{
		$sql = "SELECT DISTINCT(M.meta_value) as id_product, OI.order_item_name as product_name, M2.meta_value as price
			...
			LEFT JOIN ".$prefix."woocommerce_order_itemmeta M2 ON M2.order_item_id = OI.order_item_id AND M2.meta_key = '_line_total'
	...
	function wee_updateProduct($id_product, $num_ratings, $avg_rating)
	{
        ...
        $sql = "UPDATE wee_products SET num_ratings = '$num_ratings', avg_rating = '$avg_rating' WHERE id_product = '$id_product'"; 
	...
	function wee_insertProduct($id_product, $num_ratings, $avg_rating)
	{
        ...
        $sql = "INSERT INTO `wee_products`(id_product, num_ratings, avg_rating) VALUES ('$id_product', '$num_ratings', '$avg_rating')";
	...
	function wee_updateShopInfo()
	{
        ...
        $this->wee_getWebPage('http://weecomments.com/wsrest/update_module_version?api='.$this->weecomments_options['WEE_API_KEY'].'&version=3.1.4');

		//Si el usuario y pass son correctos, actualiza los datos de la tienda
		if ((int)$shop_info->id_shop) {
			
			$this->wee_updateShopStatistics();
            ...
            $weecomments_options['WEE_COLOUR'] = 0;
            $weecomments_options['WEE_COLOUR2'] = 0;
            $weecomments_options['WEE_MAIL_HEADER'] = 0;
            $weecomments_options['WEE_MAIL_BUTTON'] = 0;
	...
	{
		$langs_configuration = $this->wee_getWebPage('http://weecomments.com/wsrest/module_langs_configuration?api='.$this->weecomments_options['WEE_API_KEY']);
		$langs_configuration = new SimpleXMLElement($langs_configuration);
        $langs_configuration = json_decode(json_encode($langs_configuration));

		//Si el usuario y pass son correctos, actualiza los datos de la tienda
		foreach ($langs_configuration as $lang_conf) {
			$weecomments_langconf['WEE'.$lang_conf->lang.'_SUBJECT'] = (string)$lang_conf->mail_subject;
			$weecomments_langconf['WEE'.$lang_conf->lang.'_TEXT'] = (string)$lang_conf->mail_text;
			$weecomments_langconf['WEE'.$lang_conf->lang.'_EMAIL'] = (string)$lang_conf->email_reply;
			$weecomments_langconf['WEE'.$lang_conf->lang] = 'active';
		}
		update_option('weecomments_langconf', $weecomments_langconf);
	}
	
	function wee_updateShopStatistics()
	{
		global $wpdb;
		$monthly_orders = $this->wee_getNumMonthlyOrders($wpdb->prefix);
		$this->wee_getWebPage('http://weecomments.com/track/update_monthly_orders?api='.$this->weecomments_options['WEE_API_KEY'].'&num_monthly_orders='.$monthly_orders);

		$last_date = $this->wee_getWebPage("http://weecomments.com/track/get_last_statistic_day?id_shop=".$this->weecomments_options['WEE_ID_SHOP']);
		...
		while ($current_date < date("Y-m-d")) {
			...
			while ($count < 30 && $current_date <= date("Y-m-d")) {

				$count++;
				$current_date = date("Y-m-d", strtotime($current_date." +1 day"));

				$daily_orders_string .= $current_date;
				$daily_orders_string .= "@";
				$daily_orders_array = $this->wee_getNumDailyOrders($current_date, $wpdb->prefix);
				$daily_orders_string .= $daily_orders_array;
				$daily_orders_string .= ",";

				$daily_sales_string .= $current_date;
				$daily_sales_string .= "@";
				$daily_sales_array = $this->wee_getDailyOrdersSales($current_date, $wpdb->prefix);
				$daily_sales_string .= round($daily_sales_array, 2);
				$daily_sales_string .= ",";
			}

			$daily_orders_string = rtrim($daily_orders_string, ',');
			$daily_sales_string = rtrim($daily_sales_string, ',');

			echo "Enviando los pedidos de la fecha " . $current_date . "<br>";
			var_dump($daily_orders_string);
			echo "<br>";
			echo "<br>";
			var_dump($daily_sales_string);
			echo "<br>";
			echo "<br>";
			echo "<br>";

			$this->wee_getWebPage("http://weecomments.com/track/update_day_orders?id_shop=".$this->weecomments_options['WEE_ID_SHOP']."&orders_array=".$daily_orders_string);
			$this->wee_getWebPage("http://weecomments.com/track/update_day_sales?id_shop=".$this->weecomments_options['WEE_ID_SHOP']."&sales_array=".$daily_sales_string);
		}
	}
